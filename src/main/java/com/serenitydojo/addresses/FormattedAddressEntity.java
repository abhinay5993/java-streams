...
public class FormattedAddressEntity {
	...
	private static AddressLineEntity postfixNumberWithNo(String externalAddress) {
		Matcher matcher = POSTFIX_WITH_NO.matcher(externalAddress);
		matcher.find();
		return new AddressLineEntity(matcher.group(1), "No " + matcher.group(2));
	}

	private static AddressLineEntity postfixNumber(String externalAddress) {
		Matcher matcher = POSTFIX_NUMBER.matcher(externalAddress);
		matcher.find();
		return new AddressLineEntity(matcher.group(1), matcher.group(2));
	}

	private static AddressLineEntity postfixNumberWithComma(String externalAddress) {
		Matcher matcher = POSTFIX_WITH_COMMA.matcher(externalAddress);
		matcher.find();
		return new AddressLineEntity(matcher.group(1), matcher.group(2));
	}

	private static AddressLineEntity prefixNumber(String externalAddress) {
		Matcher matcher = PREFIX_NUMBER.matcher(externalAddress);
		matcher.find();
		return new AddressLineEntity(matcher.group(2), matcher.group(1));
	}
